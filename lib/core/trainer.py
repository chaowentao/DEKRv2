...
def do_train(cfg, model, data_loader, loss_factory, optimizer, epoch,
    ...
    for i, (image, heatmap, mask, offset, offset_w) in enumerate(data_loader):
        ...
        if i % cfg.PRINT_FREQ == 0 and cfg.RANK == 0:
            ...
            writer.add_scalar('train_heatmap_loss', heatmap_loss_meter.val,
                              global_steps)
            writer.add_scalar('train_offset_loss', offset_loss_meter.val,
                              global_steps)
...
def _get_loss_info(meter, loss_name):
    msg = ''
    msg += '{name}: {meter.val:.3e} ({meter.avg:.3e})\t'.format(name=loss_name,
                                                                meter=meter)
