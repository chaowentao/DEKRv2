...
class PoseHigherResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.offset_feature_layers, self.offset_final_layer = \
            self._make_separete_regression_head_part(config_offset)
    ...
    def _make_separete_regression_head_part(self, layer_config):
        offset_feature_layers = []
        offset_final_layer = []
        ...
        for _ in range(4):
            ...
            offset_conv = nn.Conv2d(
                in_channels=layer_config['NUM_CHANNELS_PERKPT'] * 3,
                out_channels=2 * 3,
                kernel_size=self.spec.FINAL_CONV_KERNEL,
                stride=1,
                padding=1 if self.spec.FINAL_CONV_KERNEL == 3 else 0)
    ...
    def forward(self, x):
        ...
        final_offset.append(self.offset_final_layer[4](
            self.offset_feature_layers[4](
                offset_feature[:, 12 * self.offset_prekpt:14 *
                               self.offset_prekpt])))
