...
class PoseHigherResolutionNet(nn.Module):
    ...
    def _make_separete_regression_head(self, layer_config):
        ...
        for i in range(self.num_joints):
            ...
            if i in [2, 8, 11, 5, 12]:
                ...
                offset_conv = nn.Conv2d(
                    in_channels=layer_config['NUM_CHANNELS_PERKPT'] * 6,
                    out_channels=2,
                    kernel_size=self.spec.FINAL_CONV_KERNEL,
                    stride=1,
                    padding=1 if self.spec.FINAL_CONV_KERNEL == 3 else 0)
            elif i in [0, 1, 3, 4, 6, 7, 9, 10, 8, 11]:
                offset_conv = nn.Conv2d(
    ...
    def forward(self, x):
        ...
        final_offset = []
        offset_feature_joint = []
        offset_feature = self.transition_offset(x)
        for j in range(self.num_joints):
            offset_feature_joint.append(self.offset_feature_layers[j](
                offset_feature[:, j * self.offset_prekpt:(j + 1) *
                               self.offset_prekpt]))

        for j in range(self.num_joints):
            if j in [2, 8, 11, 5, 12]:
                offset_feature_joint_agg = torch.cat(
                    [offset_feature_joint[j], offset_feature_joint[j - 1]],
                    dim=1)
            elif j in [13]:
                offset_feature_joint_agg = torch.cat([
                    offset_feature_joint[0], offset_feature_joint[12],
                    offset_feature_joint[13], offset_feature_joint[3],
                    offset_feature_joint[6], offset_feature_joint[9]
                ],
                                                     dim=1)
            elif j in [0, 1, 3, 4, 6, 7, 9, 10, 8, 11]:
                if j in [0, 6, 9, 3]:
                    offset_feature_joint_agg = torch.cat([
                        offset_feature_joint[j], offset_feature_joint[13],
                        offset_feature_joint[j + 1]
                    ],
                                                         dim=1)
                else:
                    offset_feature_joint_agg = torch.cat([
            ...
            final_offset.append(
                self.offset_final_layer[j](offset_feature_joint_agg))
