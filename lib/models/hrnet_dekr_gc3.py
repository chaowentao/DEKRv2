...
from .conv_block import BasicBlock, Bottleneck, AdaptBlockv2
...
class PoseHigherResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.offset_feature_layers, self.offset_final_layer = self._make_regression_head(
            config_offset)
    ...
    def _make_layer(self,
        ...
        if block_name == 'ADAPTIVE':
            print('ADAPTIVE block group conv')
            layers.append(
                block(inplanes,
                      planes,
                      stride,
                      downsample,
                      dilation=dilation,
                      deformable_groups=self.num_joints))
        else:
            layers.append(
    ...
    def forward(self, x):
        ...
        offset = self.offset_final_layer[0](self.offset_feature_layers[0](
            self.transition_offset(x)))
