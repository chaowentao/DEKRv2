...
class PoseHigherResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.part_feature_layers = self._make_regression_head_part(
            config_offset)
    ...
    def _make_heatmap_head(self, layer_config):
        ...
        feature_conv = self._make_layer(blocks_dict[layer_config['BLOCK']],
    ...
    def _make_regression_head(self, layer_config):
        offset_head_layers = []

        feature_conv = self._make_layer(
            blocks_dict[layer_config['BLOCK']],
            layer_config['NUM_CHANNELS_PERKPT'] * self.num_joints,
            layer_config['NUM_CHANNELS_PERKPT'] * self.num_joints,
            layer_config['NUM_BLOCKS'],
            dilation=layer_config['DILATION_RATE'])
        offset_head_layers.append(feature_conv)

        offset_conv = nn.Conv2d(
            in_channels=layer_config['NUM_CHANNELS_PERKPT'] * self.num_joints,
            out_channels=self.num_offset,
            groups=self.num_joints,
            kernel_size=self.spec.FINAL_CONV_KERNEL,
            stride=1,
            padding=1 if self.spec.FINAL_CONV_KERNEL == 3 else 0)
        offset_head_layers.append(offset_conv)

        return nn.ModuleList(offset_head_layers)
    ...
    def _make_separete_regression_head(self, layer_config):
        ...
        return nn.ModuleList(offset_feature_layers), nn.ModuleList(
            offset_final_layer)

    def _make_regression_head_part(self, layer_config):
        part_feature_layers = []

        for _ in range(4):
            feature_conv = self._make_layer(
                blocks_dict[layer_config['BLOCK']],
                layer_config['NUM_CHANNELS_PERKPT'] * 3,
                layer_config['NUM_CHANNELS_PERKPT'] * 3,
                layer_config['NUM_BLOCKS'],
                dilation=layer_config['DILATION_RATE'])
            part_feature_layers.append(feature_conv)
        ...
        feature_conv = self._make_layer(
            ...
            dilation=layer_config['DILATION_RATE'])
        part_feature_layers.append(feature_conv)
        return nn.ModuleList(part_feature_layers)
    ...
    def forward(self, x):
        ...
        heatmap = self.head_heatmap[1](self.head_heatmap[0](
        ...
        part_feature_list = []
        for j in range(4):
            part_feature_list.append(self.part_feature_layers[j](
                offset_feature[:, j * 3 * self.offset_prekpt:(j + 1) * 3 *
                               self.offset_prekpt]))

        part_feature_list.append(self.part_feature_layers[4](
            offset_feature[:,
                           12 * self.offset_prekpt:14 * self.offset_prekpt]))

        part_feature = torch.cat(part_feature_list, dim=1)
        for j in range(self.num_joints):
            final_offset.append(self.offset_final_layer[j](
                self.offset_feature_layers[j](
                    part_feature[:, j * self.offset_prekpt:(j + 1) *
                                 self.offset_prekpt])))
    ...
    def init_weights(self, pretrained='', verbose=True):
        ...
        if os.path.isfile(pretrained):
            ...
            for name, m in pretrained_state_dict.items():
                if name.split('.')[
                        0] in self.pretrained_layers or self.pretrained_layers[
                            0] == '*':
                    if name in parameters_names or name in buffers_names:
                        if verbose:
                            logger.info('=> init {} from {}'.format(
                                name, pretrained))
