...
def parse_args():
    ...
    parser.add_argument('--inferenceFps', type=int, default=5)
...
def main():
    ...
    if fps < args.inferenceFps:
        raise ValueError('desired inference fps is ' + str(args.inferenceFps) +
                         ' but video fps is ' + str(fps))
    skip_frame_cnt = round(fps / args.inferenceFps)
    frame_width = int(vidcap.get(cv2.CAP_PROP_FRAME_WIDTH))
    frame_height = int(vidcap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    print("fps:", fps)
    print("inferenceFps:", args.inferenceFps)
    print("skip_frame_cnt:", skip_frame_cnt)
    ...
    while vidcap.isOpened():
        ...
        for i in range(len(pose_preds)):
            ...
            show_skeleton(image_debug, pose_preds[i], thr=0.3)
    ...
    if cfg.DATASET.DATASET_TEST == 'coco':
        ...
        raise ValueError(
            'Please implement keypoint_index for new dataset: %s.' %
            cfg.DATASET.DATASET_TEST)
