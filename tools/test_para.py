...
def main_worker(gpu, ngpus_per_node, args, final_output_dir, tb_log_dir):
    ...
    #     if 'offset_feature_layers' in name:
    #         print(name, ':', parameters.size())
    #     if 'offset_final_layer' in name:
    #         print(name, ':', parameters.size())

    # def get_parameter_number(model):
    #     ada_sum = sum(p.numel() for n, p in model.named_parameters()
    #                   if 'offset_feature_layers' in n)
    #     conv_sum = sum(p.numel() for n, p in model.named_parameters()
    #                    if 'offset_final_layer' in n)
    #     total_sum = sum(p.numel() for n, p in model.named_parameters())
    #     print('ada:', ada_sum, 'conv:', conv_sum)
    ...
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        ...
        if (epoch + 1) % 50 == 0:
            if not cfg.MULTIPROCESSING_DISTRIBUTED or (
                    cfg.MULTIPROCESSING_DISTRIBUTED and args.rank == 0):
                logger.info(
                    '=> saving 50 checkpoint to {}'.format(final_output_dir))
                save_checkpoint(
                    {
                        'epoch': epoch + 1,
                        'model': cfg.MODEL.NAME,
